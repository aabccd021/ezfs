...
in
{
  ...
  testScript = ''
    for i in range(5):
      ...
      # Start only child service - parent should be mounted automatically via zfs mount -a
      ...
      server.succeed("zpool destroy spool")
      server.shutdown()

    print("All 5 runs passed - no flakiness detected")
  '';
}
